        ...
        <div className="flex min-w-0 items-center gap-3">
          {match.player?.profileIconUrl && (
            <div className="h-10 w-10 shrink-0 rounded-full bg-black/30 overflow-hidden">
                <Image src={match.player.profileIconUrl} alt="Profile Icon" width={40} height={40} className="h-full w-full object-cover" />
            </div>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-1 min-w-0">
              <div className="font-semibold text-white truncate">{match.player?.name ?? match.championName}</div>
              {match.viewedParticipant && match.ddragonVersion && (
                <LoadoutRow 
                  items={Array.isArray(match.viewedParticipant.itemsJson) ? match.viewedParticipant.itemsJson as number[] : []} 
                  spell1Id={match.viewedParticipant.spell1Id} 
                  spell2Id={match.viewedParticipant.spell2Id} 
                  augments={Array.isArray(match.viewedParticipant.augmentsJson) ? match.viewedParticipant.augmentsJson as number[] : []} 
        ...
        <div className="flex items-center justify-between gap-3 md:justify-end">
          <div className="min-w-0 text-right whitespace-nowrap">
            ...
            <div className="text-[11px] text-stone-400">
              {formatKda(match.kills, match.deaths, match.assists)}:1 {match.kp !== undefined ? `· ${match.kp}% KP` : ""}
            </div>
          </div>
