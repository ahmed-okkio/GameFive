...
// Correcting the prisma client initialization for this script
...
// The payloads extracted from diagnostic.log
const payloads = [
  {
    "gameId": 7874749978,
    ...
    uploaderPuuid: "e1d87b48-7323-5fda-9fb9-d2564745c736" // Aristo#Risto
  },
  {
    "gameId": 7874746474,
...
async function ingestMatches() {
    ...
    for (const p of payloads) {
        // Must use the real prisma client, not the one from context if this was a module import.
        // Since it's a script, it's fine.
        await ingestCompanionMayhemMatch(p as any);
        console.log(`Ingested match ${p.gameId}`);
    }
    console.log("Ingestion complete.");
}

ingestMatches().catch(console.error).finally(async () => await prisma.$disconnect());
