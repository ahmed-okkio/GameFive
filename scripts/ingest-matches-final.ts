...
import { ingestCompanionMayhemMatch } from "../lib/ingest/mayhem";

// Script to manually run the ingestion on the production database
// using the backend function directly, which handles the correct payload mapping.

const prisma = new PrismaClient({
    datasources: {
        db: {
            url: "postgresql://postgres:YasuoisGam383@db.opqknegnedfypkctcbuh.supabase.co:5432/postgres"
        }
    }
});
...
const payloads = [
    {
        ...
        "participantIdentities": [
            {"participantId": 1, "player": {"puuid": "a7f7710a-7c89-5784-ad4f-0d65e7eede57", "gameName": "El Tamator", "tagLine": "EUW"}},
            {"participantId": 2, "player": {"puuid": "2a7f2cd2-6fba-5038-a377-699f342c4d35", "gameName": "AIS Exile", "tagLine": "EUW"}},
            {"participantId": 3, "player": {"puuid": "5e39752e-8650-58aa-a9f8-a6a071b8a653", "gameName": "Snitchblasta56", "tagLine": "9380"}},
            {"participantId": 4, "player": {"puuid": "dafeef65-0ea1-52d4-bf39-7ecbfc589d72", "gameName": "Kadash", "tagLine": "TOP"}},
            {"participantId": 5, "player": {"puuid": "e1d87b48-7323-5fda-9fb9-d2564745c736", "gameName": "Aristo", "tagLine": "Risto"}},
            {"participantId": 6, "player": {"puuid": "0063a383-8f7b-5ab4-ae1a-d42b5831bb75", "gameName": "Itchy Finger", "tagLine": "EUW"}},
            {"participantId": 7, "player": {"puuid": "3f2e7fcf-3588-52a5-9c82-c496637ed660", "gameName": "iNoXaH Banchamek", "tagLine": "EUW"}},
            {"participantId": 8, "player": {"puuid": "376e050f-e47f-5da2-88c8-95488b8e7217", "gameName": "d\u00EDe for me", "tagLine": "EUW"}},
            {"participantId": 9, "player": {"puuid": "9b85a04d-2176-5d4f-ba41-5097f3d58304", "gameName": "AdoRe", "tagLine": "jxstn"}},
            {"participantId": 10, "player": {"puuid": "9d4233de-fb2f-5345-8484-ebf6903a023d", "gameName": "Chinglish", "tagLine": "11451"}}
        ]
    },
    {
        "source": "lcu",
...
// Helper to map the diagnostic log structure to the ingest API schema
function transformPayload(payload: any) {
    return {
        ...payload,
        participants: payload.participants.map((p: any) => {
            const identity = payload.participantIdentities.find((pi: any) => pi.participantId === p.participantId);
            return {
                ...p,
                ...p.stats, // Flatten stats
...
async function ingest() {
    console.log("Ingesting missing matches...");
    for (const p of payloads) {
        const payload = transformPayload(p);
...
ingest().catch(console.error).finally(async () => await prisma.$disconnect());
